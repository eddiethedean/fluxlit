...
2. Edit **`deployment.yaml`**: replace `YOUR_REGISTRY/fluxlit-app:0.5.0` with your image; set `FLUXLIT_*` and secrets via `ConfigMap` / `Secret` (do not commit real secrets).
