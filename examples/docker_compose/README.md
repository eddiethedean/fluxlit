...
- **Locked dependencies:** `requirements.txt` is generated from `requirements.in` with **`pip-compile`** (install **`pip-tools`**, then run `pip-compile requirements.in` from this directory). Regenerate when adopting a newly published FluxLit minor or refreshing transitive pins.
