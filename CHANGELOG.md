...
- **Testing:** :class:`~fluxlit.testing.FluxLitTestClient` gains :meth:`~fluxlit.testing.FluxLitTestClient.assert_no_streamlit_exception` and :meth:`~fluxlit.testing.FluxLitTestClient.select_page`; module helpers :func:`~fluxlit.testing.apptest_assert_no_errors`, :func:`~fluxlit.testing.assert_no_streamlit_exception`, and :func:`~fluxlit.testing.apptest_select_page`. The Streamlit entrypoint applies ``?page=`` via :func:`~fluxlit.deep_links.match_nav_page` before :func:`streamlit.navigation` for deep links and ``AppTest``. Closes #31.
