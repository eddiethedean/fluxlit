...
- The default **`client`** injected into pages does not send **Authorization**. Use :meth:`fluxlit.client.ApiClient.with_bearer`, :meth:`fluxlit.client.ApiClient.for_fluxlit`, an `auth_header_factory`, or {func}`fluxlit.auth.streamlit.prepare_streamlit_api_client` after OIDC — {doc}`auth-recipes` and {doc}`streamlit-api-client`.
