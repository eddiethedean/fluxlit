...
| **Strip prefix** (Caddy) | `+ docker-compose.caddy.yml` | `http://127.0.0.1:8084/myapp/` | `handle_path /myapp/*` → FluxLit. |
| **Strip prefix** (Traefik) | `+ docker-compose.traefik.yml` | `http://127.0.0.1:8085/myapp/` | File provider; `stripPrefix` + `PathPrefix(/myapp)` → FluxLit. |
...
## Traefik strip-prefix (port 8085)

```bash
docker compose -f docker/proxy-deployment/docker-compose.yml -f docker/proxy-deployment/docker-compose.traefik.yml up --build
BASE_URL=http://127.0.0.1:8085 ./docker/proxy-deployment/smoke-test.sh
```
