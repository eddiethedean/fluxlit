...
**OIDC BFF:** By default, PKCE ``state`` and Streamlit ``auth_code`` live in **process memory** via :class:`fluxlit.auth.oidc.InMemoryOIDCBFFTokenStore`. Deploy the FastAPI process with **one worker/replica** unless you inject a shared :class:`fluxlit.auth.oidc.OIDCBFFTokenStore` through :attr:`fluxlit.auth.oidc.OIDCBFFConfig.bff_token_store` (for example Redis with TTL); see ``examples/oidc_bff_pluggable_store/README.md``. ``id_token`` is validated with IdP JWKS when using ``GenericOIDCClient``.
