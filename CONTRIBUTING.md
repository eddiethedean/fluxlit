...
6. **Examples lockfile** — bump **`examples/docker_compose/requirements.in`** (for example `fluxlit>=0.11,<1.0`) and run **`uv pip compile examples/docker_compose/requirements.in -o examples/docker_compose/requirements.txt`** so the pinned example matches installable releases. If that range is not on PyPI yet, run **`uv build -o /tmp/fluxlit-wheels`** and add **`--find-links /tmp/fluxlit-wheels`** to the compile command (see the comment block in **`requirements.in`**).
